...
# Load trophy images
B_tier_trophy = pygame.image.load('B_tier.png')
B_tier_trophy = pygame.transform.scale(B_tier_trophy, (100, 100))  # Adjust size as needed

A_tier_trophy = pygame.image.load('A_tier.png')
A_tier_trophy = pygame.transform.scale(A_tier_trophy, (150, 150))  # Adjust size as needed
...
# Trophy System Variables
trophy_images = {
    'B': B_tier_trophy,
    'A': A_tier_trophy
}
current_trophy = None  # Holds the current trophy's data
trophy_spawn_tier = "B"
trophy_spawn_threshold = 2000  # Initial spawn at 20,000 meters
trophy_respawn_distance = 5000  # Respawn every 5,000 meters if not collected
last_trophy_spawn_distance = 0  # Tracks when the last trophy was spawned
trophy_display_active = False  # Indicates if the trophy is currently being displayed
trophy_slowdown_active = False
trophy_display_timer = 0  # Timer for the display duration
TROPHY_EFFECT_DURATION = fps * 5  # 10 seconds (fps frames)
trophy_speed = 5
trophies = []

# Sound effect variables
collect_energy_sound = pygame.mixer.Sound('collect_energy.wav')
collect_energy_sound.set_volume(0.7)  # 70% volume
collect_hand_watch = pygame.mixer.Sound('collect_hw.wav')
collect_hand_watch.set_volume(0.7)
collect_trophy = pygame.mixer.Sound('collect_trophy.wav')
collect_trophy.set_volume(0.7)
...
def draw_screen(line_list, lase):
    ...
    if not pause:
        lase[0][0] -= (laser_speed * speed_multiplier) + game_speed
        lase[1][0] -= (laser_speed * speed_multiplier) + game_speed
    ...
    if lase[0][1] == lase[1][1]:  # Horizontal laser
        ...
        laser_rect = pygame.Rect(lase[0], (laser_vertical_image.get_width(), height - (20 * (height != laser_sprite_height))))
        laser_img = pygame.Surface((laser_rect.w, laser_rect.h), pygame.SRCALPHA)
        positions = [
            (0, (laser_sprite_height - 20) * i)
            for i in range(round(height/laser_sprite_height))
        ]
...
def draw_energy_drinks():
    ...
    for drink in energy_drinks[:]:  # Loop through a copy of the list
        ...
        if not pause:
            ...
            drink[0] -= (energy_drink_speed * speed_multiplier) + game_speed
...
def draw_hand_watches():
    ...
    for watch in hand_watches[:]:  # Loop through a copy of the list
        ...
        if not pause:
            ...
            watch[0] -= (watch_speed * speed_multiplier) + game_speed
...
def activate_hand_watch_power():
    ...
    if not hand_watch_active:  # Activate only if not already active
        ...
        # print("Hand watch activated. Game speed is now:", game_speed * 0.2)
    collect_hand_watch.play()

# draw player including animated states
def draw_player():
    global player_y, counter
    play = pygame.rect.Rect((100, player_y + 10), (player_standing.get_width(), player_standing.get_height()))
...
def generate_laser():
    laser_type = 1 # random.randint(0, 1)
...
def draw_rocket(coords, mode):
    if mode == 0:
        ...
        if not pause:
            coords[0] -= (rocket_speed * speed_multiplier) + game_speed
...
# --- Trophy System Functions ---
def spawn_trophy(tier='B'):
    global current_trophy, last_trophy_spawn_distance

    # Choose the trophy image based on tier
    trophy_image = trophy_images[tier]

    # Define trophy position (off-screen to the right)
    trophy_x = WIDTH + random.randint(50, 300)  # Adjust as needed
    trophy_y = random.randint(100, HEIGHT - 150)  # Adjust as needed

    # Create a Rect for collision detection
    trophy_rect = trophy_image.get_rect(topleft=(trophy_x, trophy_y))

    # Store current trophy data
    current_trophy = {
        'type': tier,
        'image': trophy_image,
        'rect': trophy_rect
    ...
    last_trophy_spawn_distance = distance
    return current_trophy

def update_and_draw_trophy():
    global trophies, trophy_display_active, trophy_display_timer, trophy_spawn_tier, trophy_spawn_threshold

    for trophy in trophies[:]:  # Loop through a copy of the list
        # Draw the trophy
        screen.blit(trophy['image'], trophy['rect'])

        if not pause:
            # Move trophy to the left
            trophy['rect'].x -= (trophy_speed * speed_multiplier) + game_speed

        # Remove the trophy if it goes off-screen
        if trophy['rect'].x < -trophy['rect'].width:
            trophies.remove(trophy)
            # print(f"Trophy went off-screen without being collected.")
            continue  # Skip collision check for removed trophy

        # Check collision with player
        player_rect = pygame.Rect(100, player_y, player_standing.get_width(), player_standing.get_height())
        trophy_rect = trophy['rect']
        if player_rect.colliderect(trophy_rect):
            activate_trophy()
            trophies.remove(trophy)

            if trophy['type'] == 'B':
                trophy_spawn_tier = "A"
                trophy_spawn_threshold = distance + 15000


def activate_trophy():
    global trophy_slowdown_active, trophy_slowdown_timer
    global trophy_display_active, trophy_display_timer
    global speed_multiplier

    # Activate Slowdown Effect
    if not trophy_slowdown_active:
        trophy_slowdown_active = True
        trophy_slowdown_timer = TROPHY_EFFECT_DURATION  # e.g., 10 seconds
        # print(f"Trophy slowdown activated. speed_multiplier set to {speed_multiplier}")

    # Activate Trophy Image Display
    trophy_display_active = True
    trophy_display_timer = TROPHY_EFFECT_DURATION  # Same duration as slowdown
    # print("Trophy image overlay activated.")
...
def update_game_state():
    global hand_watch_active, hand_watch_timer
    global trophy_slowdown_active, trophy_slowdown_timer
    global trophy_display_active, trophy_display_timer
# If hand watch is active, decrease the timer
    ...
    if trophy_slowdown_active :
        if trophy_slowdown_timer > 0:
            trophy_slowdown_timer -= 1
        else:
            trophy_slowdown_active = False
    if trophy_display_active:
        if trophy_display_timer > 0:
            trophy_display_timer -= 1
        else:
            trophy_display_active = False
...
while run:
    ...
    draw_energy_drinks()
    draw_hand_watches()
    update_and_draw_trophy()

    if (distance - last_trophy_spawn_distance >= trophy_respawn_distance and distance >= trophy_spawn_threshold) or (distance >= trophy_spawn_threshold and current_trophy == None):
        trophies.append(spawn_trophy(trophy_spawn_tier))
    ...
    for blade in blades:
        ...
        if not pause:
            blade[0] -= (blade_speed * speed_multiplier) + game_speed  # Move blade to the left only if the game is not paused
    ...
    if trophy_display_active:
        ...
        overlay.fill((0, 0, 0, 180))  # Black with alpha for dimming
        screen.blit(overlay, (0, 0))

        # Scale the trophy image to cover most of the screen
        scaled_image = pygame.transform.scale(current_trophy["image"], (int(WIDTH * 0.8), int(HEIGHT * 0.8)))
    ...
    if restart_cmd:
        ...
        current_trophy = None
        trophy_display_active = False
        trophy_display_timer = 0
        trophy_slowdown_active = False
        trophy_slowdown_timer = 0
        trophy_spawn_threshold = 2000
        last_trophy_spawn_distance = 0
        trophy_spawn_tier = "B"
